...
function LoadUsers() {
    ...
    if (user.id == c.creatorId) {
        ...
        for (it in users) {
            if (users[it].canvasId == params) canvasRelatedUsers.push(users[it].userId);
        }

        var x = "";
        document.getElementById("deleteUsersPicker").innerHTML = x;
        for (it in canvasRelatedUsers) {
            x += "<option value=\"" + canvasRelatedUsers[it] + "\">" + GetUserEmail(canvasRelatedUsers[it], logins) + "<\/option>";
        }
        document.getElementById("deleteUsersPicker").innerHTML += x;
