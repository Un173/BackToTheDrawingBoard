function LoadCanvases() {
    ...
    xmlhttp.onload = function (ev) {
        ...
        if (xmlhttp.status == 200)
        {
            ...
            request.open("GET", "api/CanvasUsers/", false);
            request.send();
            var users = JSON.parse(request.responseText);

            for (i in canvases)
            {
                if (canvases[i].creatorId == currentUser.id || CheckUserAllowance(users, canvases[i], currentUser))//Или разрешено
...
function CheckUserAllowance(users, canvas, currentUser)
{
    for (it in users)
    {
        if (users[it].canvasId == canvas.id && users[it].userId == currentUser.id) return true;
